...
//            dual_r[1:0] <= {rt[60], ~rt[60]};
//            dual_r[3:2] <= {rt[61], ~rt[61]};
        end else begin
            dual_x <= 0;
            dual_y <= 0;
            dual_y2 <= 0;
//            dual_r <= 0;
    ...
    always@(posedge clk or negedge rst_n) begin 
        if (!rst_n) begin 
            ...
            dual_ru <= dual_u ^ {2*K{rt[60]}};
            dual_rv <= dual_v ^ {2*K{rt[61]}};
            dual_s <= dual_c ^ {2{rt[62]}};
        end else begin
            dual_ru <= 0;
            dual_rv <= 0;
            dual_s <= 0;
...
//        rt[60 +: 60],
    ...
    for (i = 0; i < K; i = i + 1) begin
        ...
        assign xB[K+i] = rt[63] ^ rt[64] ^ rf[2*K+i];
    end
