...
(
    ...
    output reg succ_exec
     ...
     wire [75*8-1:0] tb_and_0;
     wire [75*8-1:0] tb_and_1;
     wire [8*2*K-1:0] tb_and_2;
    (* DONT_TOUCH="true" *) wire [K-1:0] u;
    (* DONT_TOUCH="true" *) wire [K-1:0] v;
    (* DONT_TOUCH="true" *) wire c;
    (* DONT_TOUCH="true" *) reg [75*8-1:0] r_tb_and_0;
    (* DONT_TOUCH="true" *) reg [75*8-1:0] r_tb_and_1;
    ...
    always@(posedge clk or negedge rst_n) begin 
        if (!rst_n) begin 
            ...
            for (k = 0; k < 2*K; k = k + 2) begin
                ...
                dual_y2[k+1] <= xA2[k/2] ^ rf[2*K+k/2];
                dual_y2[k] <= ~xA2[k/2] ^ rf[2*K+k/2];
            end 
        end else begin
            dual_x <= 0;
            dual_y <= 0;
...
//        rt[60 +: 60],
    ...
    wire [N*K-1:0] xB;
    for (i = 0; i < K; i = i + 1) begin
        assign xB[i] = dual_xB[2*i+1] ^ rf[3*K+i];
        assign xB[K+i] = rt[123] ^ rt[124] ^ rf[3*K+i];
    end
