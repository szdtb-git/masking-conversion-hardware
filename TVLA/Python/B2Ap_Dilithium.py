...
for alun in range(0, zongshu // numEachFile):
    ...
    for j in trange(numEachFile):
        ...
        for i in range(0,44):
            ramaa = hex(np.random.randint(0,255)).replace('0x','')
            if(len(ramaa)<2):
                ramaa = '0'*(2-len(ramaa))+ramaa
            print(ramaa)
            ramaa=bytes.fromhex(ramaa)
    ...
    np.save(r'E:/A2B/LUT_B2Ap_Dilithium_off/chufa/arrPart{0}.npy'.format(alun+file_number), arr)
    # np.save(r'E:/A2B/LUT_B2Ap_Kyber/chufa/arr_trigger_Part{0}.npy'.format(alun+file_number), arr_tr)
    fl.close()
    del arr
    del arr_tr
